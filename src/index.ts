...
function apexHost(host: string): string {
  ...
  if (h.startsWith("dev.")) h = h.slice(4)
  if (h.startsWith("www.")) h = h.slice(4)
  return h
}
...
async function computeContentCode(articlePath: string): Promise<string> {
...
function readEnv() {
  ...
  return {
    ...
    notFoundPath:
      process.env.ORIONE_CONTENT_LINK_NOT_FOUND_PATH?.trim() || "/not-found",
...
type ContentLinkApi =
  | { found: true; long_url: string }
  | { found: false }
  | "unavailable"
  | "unauthorized"

async function fetchContentLink(
  code: string,
  host: string,
  token: string,
  apiBase: string
): Promise<ContentLinkApi> {
  ...
  return (await res.json()) as { found: true; long_url: string } | { found: false }
}

function articleUrl(host: string, path: string): string {
...
async function findLocalPath(
  ...
  if (!articlePaths?.length) return null
  for (const p of articlePaths) {
    if ((await computeContentCode(p)) === code) return normalizeArticlePath(p)
  }
  return null
}

async function resolveContentRedirect(
  request: NextRequest,
  code: string
  ...
  if (api === "unauthorized") {
    return new NextResponse(
      "Invalid ORIONE_CONTENT_LINK_TOKEN (need Samples API key with content-links:read)",
      { status: 401 }
    )
  }
  ...
  if (api === "unavailable") {
    ...
    if (local) longUrl = articleUrl(host, local)
  } else if (api.found) {
 ...
 */
export async function handleContentLink(
  request: NextRequest
...
export type ContentLinkFallback = (
  request: NextRequest
...
export function createMiddleware(options?: {
  ...
  return async function contentLinkMiddleware(request: NextRequest) {
    const hit = await handleContentLink(request)
    if (hit) return hit
